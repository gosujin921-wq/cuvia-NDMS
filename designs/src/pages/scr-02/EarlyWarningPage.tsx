    ...
    <div className="relative h-full w-full overflow-hidden">
      ...
      <div className={UTIL_STRIP}>
        <MapUtilStrip
          ...
          layers={[
            ...
            /* 위험요소 — 장비와 별도 그룹(03 §2). 행안부 생활안전지도 실 자료다.
               기본 켜짐이라 지구를 열면 "왜 위험지구인가"가 지도에 이미 깔려 있다 */
            {
              title: "위험요소",
      ...
      {event && view && risk && effectiveLevel && hero && (
        <ExecutionPopup
          ...
          district={district}
          event={event}
          levelLabel={levelSpec(view.level).label}
          levelColor={levelSpec(view.level).color}
          valueText={`${view.value} ${event.unit}`}
          processState={processStateAt(event, now, approvedResponseLevel)}
          risk={risk}
          approved={approved}
          onApprove={approveResponseLevel}
