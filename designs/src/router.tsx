/* ─────────────────────────────────────────────
 * 라우터 — SCR-01~06 (SCR-03 은 결번 · 02 §4)
 *
 * URL · 페이지 폴더 · 사이드바 nav.ts 가 전부 SCR 번호로 1:1 정렬된다.
 * 화면을 추가하면 nav.ts · 페이지 폴더 · 이 파일을 같은 번호로 함께 세운다.
 * 배경: docs/레거시/정본/02_IA_화면구조.md
 * ───────────────────────────────────────────── */
...
function LegacyDispatchRedirect() {
  const [params] = useSearchParams();
  const requested = params.get("event");
  const event = EVENTS.find((e) => e.id === requested) ?? null;
  return (
    <Navigate to={event ? `/scr-02/${event.districtId}?event=${event.id}` : "/scr-02"} replace />
  );
}
...
export const router = createBrowserRouter([
  ...
  { path: "/", element: <Navigate to={HUB_ROUTE} replace /> },
  {
    ...
    children: [
      ...
      { path: "/scr-03", element: <LegacyDispatchRedirect /> },
