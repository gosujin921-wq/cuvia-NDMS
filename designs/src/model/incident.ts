 ...
 *  `확인됨`은 대응중에 합쳤고 `안정`은 상태가 아니라 대응중 안의 국면(IncidentPhase)이다 */
export type WorkflowStatus = "후보" | "확인중" | "대응중" | "종료" | "오탐" | "병합됨";
/** 대응중 안의 국면 — 확대가 멈추고 잔여 대응·감시만 남았을 때 `안정`(상황 안정). 보고용 꼬리표이지 승인 경계가 아니다.
 *  들어가는 것은 담당자, 풀리는 것은 시스템이다. 새 이벤트로 위험등급이 오르면 CUVIA 규칙이 국면을 비운 상태 변경을 남긴다(01 §7.4).
 *  옛 이름 `통제`는 SOP 의 도로 통제·진입 통제와 겹쳐 "도로를 막으라"로 읽혀 바꿨다 (2026-09-16 사용자 결정) */
export type IncidentPhase = "안정";
