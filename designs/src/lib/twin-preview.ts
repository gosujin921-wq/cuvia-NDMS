/* ─────────────────────────────────────────────
 * 대안은 비교다 — 디지털트윈에서 대응 버튼은 "하면 어떻게 되나"이고 실행이 아니다 (2026-09-15 확정)
 *
 * 예측판(fixture)은 대안이 실행된 세계를 적어 둔다("통제됨"). 화면에서는 그 말을 그대로 쓰면 실행된 것으로 읽힌다.
 * 그래서 올리기 전에 한 번 바꾼다 — 통제됨 → 통제 시 · 통제 가정. 실제 실행은 사건 안의 대응 화면(IA-04)이 하고 그때의 말은 그쪽이 든다.
 * 값은 안 바꾼다. 말만 바꾼다.
 * ───────────────────────────────────────────── */
