...
수정 대상은 이 저장소뿐이다. 워크스페이스 사이드바에서 `▶` 가 붙은 폴더 하나만 작업 대상이고, `· ref/` 가 붙은 폴더(IDC · platform_web · secon)는 **읽기 전용 참조**다. 저장소 안의 `from-DSMS-main/` 도 다른 작업자의 저장소를 원본 그대로 보존한 것이라 읽기 전용이다. 읽어서 가져오되 고치지 않는다. 참조 쪽 수정이 필요하면 먼저 알린다.
